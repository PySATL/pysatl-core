...
class TransformationOperatorsMixin:
    ...
    def _affine_transform(self, *, scale: float, shift: float) -> Distribution:
        ...
        return affine(
            ...
            scale=scale,
            shift=shift,
            methods=default_affine_transformation_methods(
    ...
    def _binary_transform(
        ...
        *,
        ...
        """
        from pysatl_core.distributions.distribution import Distribution
        from pysatl_core.transformations.operations.distributions.binary.base import binary
        ...
        if operation in {BinaryOperationName.ADD, BinaryOperationName.SUB}:
            methods = default_linear_binary_transformation_methods(kind=kind)
        elif operation == BinaryOperationName.MUL:
            methods = default_multiplication_binary_transformation_methods(kind=kind)
        else:
            methods = default_division_binary_transformation_methods(kind=kind)
    ...
    def __radd__(self, other: object) -> Distribution | NotImplementedType:
        ...
        if isinstance(other, Real):
            return self._affine_transform(scale=1.0, shift=float(other))
        if isinstance(other, Distribution):
            kind = getattr(other.distribution_type, "kind", None)
            return binary(
                other,
                cast(Distribution, self),
                operation=BinaryOperationName.ADD,
                methods=default_linear_binary_transformation_methods(kind=kind),
            )
        return NotImplemented
    ...
    def __rsub__(self, other: object) -> Distribution | NotImplementedType:
        ...
        if isinstance(other, Distribution):
            ...
            return binary(
                other,
                cast(Distribution, self),
                operation=BinaryOperationName.SUB,
                methods=default_linear_binary_transformation_methods(kind=kind),
            )
        return NotImplemented
