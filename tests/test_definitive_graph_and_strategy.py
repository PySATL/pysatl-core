...
from pysatl_core.distributions.distribution import StandaloneEuclidianDistribution
from pysatl_core.distributions.registry import distribution_type_register
from pysatl_core.types import EuclidianDistributionType, GenericCharacteristicName, Kind
...
def test_definitive_graph_full_connectivity() -> None:
    dt = EuclidianDistributionType(kind=Kind.CONTINUOUS, dimension=1)
...
def test_strategy_uniform_ppf_only() -> None:
    dt = EuclidianDistributionType(kind=Kind.CONTINUOUS, dimension=1)
    distr = StandaloneEuclidianDistribution(
