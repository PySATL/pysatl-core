...
class TestFiniteMixtureDistribution(DistributionTestBase):
        ...
        *,
        ...
        analytical_computations: dict[str, AnalyticalComputation[Any, Any]] = {
            CharacteristicName.PPF: AnalyticalComputation[float, float](
                target=CharacteristicName.PPF,
                func=cast(ComputationFunc[float, float], lambda data, **_options: float(data)),
            )
        }
    ...
    def test_mixture_var_requires_only_mean_and_variance(self) -> None:
        ...
        assert mixture.query_method(CharacteristicName.VAR)() == pytest.approx(2.56)

    def test_mixture_skew_requires_only_mean_variance_and_skew(self) -> None:
        first = self._make_statistics_distribution(mean=0.0, variance=1.0, skewness=0.5)
        second = self._make_statistics_distribution(mean=1.0, variance=0.5, skewness=-0.2)
        mixture = finite_mixture([(0.5, first), (0.5, second)])
        assert set(mixture.analytical_computations) == {
            CharacteristicName.MEAN,
            CharacteristicName.VAR,
            CharacteristicName.SKEW,
        }
        assert np.isfinite(mixture.query_method(CharacteristicName.SKEW)())

    def test_mixture_kurt_requires_all_moment_statistics(self) -> None:
        first = self._make_statistics_distribution(
            mean=0.0,
            variance=1.0,
            skewness=0.5,
            kurtosis=3.5,
        )
        second = self._make_statistics_distribution(
            mean=1.0,
            variance=0.5,
            skewness=-0.2,
            kurtosis=2.8,
        ...
        kurt = mixture.query_method(CharacteristicName.KURT)
    ...
    def test_component_and_weight_accessors_are_safe(self) -> None:
        ...
        assert mixture.parent_roles == ("component_0", "component_1")
    ...
    def test_manual_methods_replace_builtin_mixture_set(self) -> None:
        first = self._make_uniform_full_distribution(0.0, 1.0)
        second = self._make_uniform_full_distribution(1.0, 2.0)
        ...
        def _custom_cdf(
            ...
            first_cdf = cast(
                Method[NumericArray, NumericArray], sources["component_0"][CharacteristicName.CDF]
            )

            def _cdf(data: NumericArray, **options: Any) -> NumericArray:
                return cast(NumericArray, np.asarray(first_cdf(data, **options), dtype=float))
        ...
        methods: TransformationMethodSpecsMap = {
            CharacteristicName.CDF: {
                DEFAULT_ANALYTICAL_COMPUTATION_LABEL: (
                    {
                        "component_0": (CharacteristicName.CDF,),
                        "component_1": (CharacteristicName.CDF,),
                    },
                    _custom_cdf,
                )
            }
        }
        mixture = finite_mixture([(0.25, first), (0.75, second)], methods=methods)

        assert set(mixture.analytical_computations) == {CharacteristicName.CDF}
        np.testing.assert_allclose(
            np.asarray(mixture.query_method(CharacteristicName.CDF)(np.asarray([0.5]))),
            np.asarray([0.5]),
        )
    ...
    def test_sample_selects_components_by_weights(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        monkeypatch.setattr(
            "pysatl_core.transformations.operations.distributions.mixture.np.random.default_rng",
            lambda: _FakeRng(),
        )
