...
from pysatl_core.transformations.operations.distributions.binary import (
...
class TestBinaryDistribution(DistributionTestBase):
    ...
    def test_add_continuous_exposes_full_characteristic_set(self) -> None:
        ...
        assert transformed.parent_roles == ("left", "right")
    ...
    def test_linear_var_can_be_built_from_variances_only(self) -> None:
        left = self._make_statistics_distribution(variance=1.0)
        right = self._make_statistics_distribution(variance=0.25)

        transformed = binary(left, right, operation=BinaryOperationName.ADD)
        assert isinstance(transformed, LinearBinaryDistribution)
        assert set(transformed.analytical_computations) == {CharacteristicName.VAR}
        assert transformed.query_method(CharacteristicName.VAR)() == pytest.approx(1.25)

    def test_linear_skew_can_be_built_without_parent_means(self) -> None:
        left = self._make_statistics_distribution(variance=1.0, skewness=1.0)
        right = self._make_statistics_distribution(variance=0.25, skewness=-0.5)
        ...
        assert np.isfinite(transformed.query_method(CharacteristicName.SKEW)())

    def test_linear_kurt_can_be_built_without_parent_means_and_skew(self) -> None:
        left = self._make_statistics_distribution(variance=1.0, kurtosis=3.4)
        right = self._make_statistics_distribution(variance=0.25, kurtosis=2.6)
        ...
        assert np.isfinite(transformed.query_method(CharacteristicName.KURT)())
    ...
    def test_manual_methods_replace_builtin_binary_set(self) -> None:
        left = self._make_uniform_full_distribution(0.0, 1.0)
        right = self._make_uniform_full_distribution(1.0, 2.0)
        ...
        def _custom_cdf(
            ...
            left_cdf = cast(Method[float, float], sources["left"][CharacteristicName.CDF])

            def _cdf(data: float, **options: Any) -> float:
                return float(left_cdf(data, **options))
        ...
        methods: TransformationMethodSpecsMap = {
            CharacteristicName.CDF: {
                DEFAULT_ANALYTICAL_COMPUTATION_LABEL: (
                    {"left": (CharacteristicName.CDF,)},
                    _custom_cdf,
                )
            }
        }
        transformed = binary(left, right, operation=BinaryOperationName.ADD, methods=methods)

        assert isinstance(transformed, LinearBinaryDistribution)
        assert set(transformed.analytical_computations) == {CharacteristicName.CDF}
        assert transformed.query_method(CharacteristicName.CDF)(0.5) == pytest.approx(0.5)
