...
from tests.utils.mocks import MockSamplingStrategy, StandaloneEuclideanUnivariateDistribution


class TestAffineDistribution(DistributionTestBase):
    def test_parent_roles_exposed_via_public_property(self) -> None:
        base = self.make_logistic_cdf_distribution()
        transformed = affine(base, scale=2.0, shift=1.0)

        assert transformed.parent_roles == ("base",)
    ...
    def test_manual_methods_replace_builtin_affine_set(self) -> None:
        ...
        base = StandaloneEuclideanUnivariateDistribution(
        ...
        def _custom_cdf(
            ...
            assert hasattr(owner, "scale")
            assert hasattr(owner, "shift")
            base_cdf = cast(Method[float, float], sources["base"][CharacteristicName.CDF])
            scale = cast(float, owner.scale)
            shift = cast(float, owner.shift)

            def _transformed(data: float, **options: Any) -> float:
                return float(base_cdf((data - shift) / scale, **options))

            return cast(ComputationFunc[float, float], _transformed)

        methods: TransformationMethodSpecsMap = {
            CharacteristicName.CDF: {
                DEFAULT_ANALYTICAL_COMPUTATION_LABEL: (
                    {"base": (CharacteristicName.CDF,)},
                    _custom_cdf,
                )
            }
        }

        transformed = affine(base, scale=2.0, shift=0.5, methods=methods)

        assert set(transformed.analytical_computations) == {CharacteristicName.CDF}
        assert transformed.query_method(CharacteristicName.CDF)(0.5) == pytest.approx(0.0)
