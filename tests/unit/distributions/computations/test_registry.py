"""
Tests for FitterRegistry: matching, selection, and tag filtering.
"""
...
def _make_desc(
    ...
    return FitterDescriptor(
        ...
        constraint_tags=constraint_tags or frozenset(),
    )
...
class TestFitterRegistry:
    ...
    def test_find_returns_none_when_tags_dont_match(self) -> None:
        reg = FitterRegistry()
        desc = _make_desc("pdf_to_cdf", constraint_tags=frozenset({"continuous"}))
        reg.register(desc)
        found = reg.find(
    ...
    def test_find_matches_with_tag_superset(self) -> None:
        reg = FitterRegistry()
        desc = _make_desc("pdf_to_cdf", constraint_tags=frozenset({"continuous", "univariate"}))
        reg.register(desc)
        found = reg.find(
            CharacteristicName.CDF,
            [CharacteristicName.PDF],
    ...
    def test_find_returns_first_registered(self) -> None:
        reg = FitterRegistry()
        first = _make_desc("first")
        second = _make_desc("second")
        reg.register(first)
        reg.register(second)
        found = reg.find(CharacteristicName.CDF, [CharacteristicName.PDF])
        assert found is first

    def test_find_all_returns_registration_order(self) -> None:
        reg = FitterRegistry()
        d1 = _make_desc("a")
        d2 = _make_desc("b")
        d3 = _make_desc("c")
    ...
    def test_find_all_filters_by_tags(self) -> None:
        reg = FitterRegistry()
        cont = _make_desc("cont", constraint_tags=frozenset({"continuous"}))
        disc = _make_desc("disc", constraint_tags=frozenset({"discrete"}))
        reg.register(cont)
        reg.register(disc)
        result = reg.find_all(
            CharacteristicName.CDF,
