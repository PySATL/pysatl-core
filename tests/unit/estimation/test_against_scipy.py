...
class TestAgainstScipy:
    ...
    def test_exponential(self, exponential_family, rng):
        # SciPy parametrises by scale = 1 / lambda, and its 'loc' has to be
        # pinned at zero: this family's support starts there by definition,
        # while SciPy would otherwise shift it to min(x).
    ...
    def test_uniform(self, uniform_family, rng):
        # SciPy parametrises by loc = a and scale = b - a.
    ...
    def test_gamma_log_likelihood_is_not_worse_than_scipy(self, gamma_family, rng):
        ...
        result = gamma_family.fit(sample)
        # A numeric-versus-numeric comparison is only meaningful in one
        # direction: whichever estimate is better must have the larger
        # likelihood, up to floating-point noise.
