...
class TestResolveBounds:
    ...
    def test_a_family_without_bounds_warns_and_runs_unbounded(self, uniform_family):
        with pytest.warns(UserWarning, match="declares no 'param_bounds'"):
            assert ParameterBox.of(uniform_family).as_scipy() is None
...
class TestClipToBounds:
    def test_moves_a_point_into_the_box(self, normal_family):
        clipped = ParameterBox.of(normal_family).clip(normal_family.base(mu=1.0, sigma=-5.0))
    ...
    def test_leaves_an_unbounded_family_alone(self, uniform_family):
        ...
        assert ParameterBox.of(uniform_family).clip(params) is params
...
class TestPropagationToViews:
    def test_a_view_inherits_the_closed_form_and_the_bounds(self, normal_family):
        ...
        assert view.closed_forms == normal_family.closed_forms
    ...
    def test_a_view_fixed_in_another_parametrization_drops_the_bounds(self, normal_family):
        ...
        with pytest.warns(UserWarning, match="declares no 'param_bounds'"):
            assert ParameterBox.of(view).as_scipy() is None


class TestDeclarationAccessors:
    """``closed_forms`` and ``base_score`` expose what the family declared.
    ...
    def test_a_family_with_a_closed_form_exposes_it(self, normal_family):
        assert callable(normal_family.closed_form_for("mle"))

    def test_a_family_without_a_closed_form_reports_none(self, gamma_family):
        assert gamma_family.closed_form_for("mle") is None
        assert gamma_family.closed_forms == {}

    def test_a_method_the_family_never_heard_of_reports_none(self, normal_family):
        # A missing key is an ordinary answer, not an error: the asking method
        # then does whatever it does numerically.
        assert normal_family.closed_form_for("mme") is None
