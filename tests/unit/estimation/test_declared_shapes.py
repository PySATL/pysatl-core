...
class TestSupportShapeProtocols:
    ...
    def test_a_lattice_support_matches_neither(self):
        # It may be infinite, so it has no point array to hand out, and it has
        # no endpoints either. Landing in neither protocol is correct.
...
class TestSupportSignature:
    ...
    def test_an_unrecognised_support_falls_back_to_an_opaque_signature(self):
        ...
        # The last resort compares printed forms, and says so in its name
        # rather than hiding behind the string "repr" inside a tuple.
...
class TestOptimizerOutcome:
    ...
    def test_an_unreported_verdict_stays_a_third_state(self):
        # Not False: "the solver said nothing" and "the solver said no" are
        # different, and the message wording downstream depends on which.
        result = OptimizeResult()
    ...
    def test_an_estimate_that_is_not_an_array_is_refused(self):
        # A solver returning a string for 'x' used to reach np.asarray and fail
        # there with numpy's wording, naming neither the solver nor the field.
        result = OptimizeResult()
    ...
    def test_an_uninterpretable_counter_reads_as_not_reported(self):
        # Guessing a number here would put a fabricated count in MLEResult.
        result = OptimizeResult()
        result["x"] = np.array([1.0])
        result["nit"] = "several"
...
class TestOptimizerName:
    ...
    def test_a_callable_without_a_name_falls_back_to_its_repr(self):
        # A callable instance carries no __name__, which is why the protocol
        # check exists instead of a getattr with a default.
        class Solver:
            def __call__(
...
class TestResultValidation:
    def test_a_sample_size_of_zero_is_refused_where_it_is_written(self, normal_family):
        # Left unchecked this surfaced much later, as 'expected a positive
        # input' from math.log inside 'bic', naming neither field nor object.
    ...
    def test_the_distribution_comes_from_the_parametrization_not_the_label(
        ...
        # 'family_name' is a label; '__family__' is the object the estimate was
        # produced by, so the two cannot drift apart here.
...
class TestFieldNames:
    ...
    def test_a_non_parametrization_is_refused_rather_than_read_as_empty(self):
        # It used to answer (), which downstream reads as "this family
        # estimates zero parameters" — a different statement entirely.
