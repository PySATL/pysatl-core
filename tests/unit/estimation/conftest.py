...
def declared_bounds(family: ParametricFamily) -> list[tuple[float, float]]:
    """``ParameterBox.as_scipy`` for a family that is expected to declare some.

    Keeps the ``None`` branch — which means "this family declares nothing" —
    out of tests that are about the values themselves.
    """
    from pysatl_core.estimation import ParameterBox

    bounds = ParameterBox.of(family).as_scipy()
...
def use_moment_start(
    monkeypatch: pytest.MonkeyPatch, family: ParametricFamily, rule: MomentRule
) -> None:
    """Give a built-in family a different moment rule for the duration of a test.

    The rule lives on the family now, and the built-in families are shared
    singletons from the register, so there is no dictionary to patch an entry
    into.  ``monkeypatch.setattr`` on the private slot is what replaces that,
    and it restores the original afterwards — which the old registry did not do
    on its own and every test had to remember.
    """
    monkeypatch.setattr(family, "_moment_start", rule)


def _make_boxed_family(name: str, moment_start: MomentRule | None = None) -> ParametricFamily:
...
def boxed_family():
    ...
    return _make_boxed_family("BoxedNoFormula", moment_start=rule)
