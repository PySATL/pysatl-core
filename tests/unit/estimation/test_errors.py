...
class TestSampleValidation:
    ...
    def test_a_view_needs_fewer_observations(self, normal_family):
        # One free parameter, so a single observation is enough to try.
...
class TestFixedSupport:
    ...
    def test_a_moving_support_charges_a_penalty_instead(self, uniform_family, rng):
        # Nothing here is outside "the" support, because the support is not
        # settled until the parameters are: this must not raise.
...
class TestStartingPointFailure:
    def test_an_unusable_start_is_reported_clearly(self, make_boxed_family, rng):
        # Without a moments rule this family starts at (1, 1), which violates
        # 'low < high', so the objective is infinite there.
